...
export class AndroidStudioJumper implements IIdeJumper {
  constructor(
  ...
  async jump(location: CodeLocation): Promise<void> {
    const result = await this.runner.run({
      ...
      args: ["--line", String(location.line), location.filePath]
    })

    if (result.exitCode !== 0) {
      throw new Error(result.stderr || `studio exitCode=${String(result.exitCode)}`)
    }
  }
}
